...
if (!isset($filterprefix) || (strlen($filterprefix)==0)) {
	...
	$QUERY_INVOICE_ENOUGH_PAID = "SELECT DATE_FORMAT(sub.date,'%c'),SUM(sub.test) FROM
		(SELECT cc_invoice.date date, IF(IFNULL(SUM(CEIL(cc_invoice_item.price*(1+(cc_invoice_item.vat/100))*100)/100),0) <= IFNULL(SUM(CEIL(cc_logpayment.payment*100)/100),0),1,0) test
			FROM cc_invoice
			...
			WHERE cc_card.id_diller = " . $_SESSION['card_id'] . " AND cc_invoice.date >= TIMESTAMP('$checkdate') AND cc_invoice.date <= CURRENT_TIMESTAMP
	...
	$QUERY_INVOICE_PAID = "SELECT DATE_FORMAT(date,'%c'),COUNT(*) FROM cc_invoice
		LEFT JOIN cc_card ON cc_card.id=cc_invoice.id_card
		WHERE cc_card.id_diller = " . $_SESSION['card_id'] . " AND paid_status = 1 AND cc_invoice.date >= TIMESTAMP('$checkdate') AND cc_invoice.date <= CURRENT_TIMESTAMP
		GROUP BY MONTH(date)
		ORDER BY date DESC";
	$result_invoice_paid = $table->SQLExec($HD_Form->DBHandle, $QUERY_INVOICE_PAID);
	
	$QUERY_INVOICE_UNPAID = "SELECT DATE_FORMAT(date,'%c'),COUNT(*) FROM cc_invoice
		LEFT JOIN cc_card ON cc_card.id=cc_invoice.id_card
		WHERE cc_card.id_diller = " . $_SESSION['card_id'] . " AND paid_status = 0 AND cc_invoice.date >= TIMESTAMP('$checkdate') AND cc_invoice.date <= CURRENT_TIMESTAMP
		GROUP BY MONTH(date)
		ORDER BY date DESC";
	$result_invoice_unpaid = $table->SQLExec($HD_Form->DBHandle, $QUERY_INVOICE_UNPAID);
	
	$QUERY_INVOICE_COUNT = "SELECT DATE_FORMAT(date,'%c'),COUNT(*) FROM cc_invoice
		LEFT JOIN cc_card ON cc_card.id=cc_invoice.id_card
		WHERE cc_card.id_diller = " . $_SESSION['card_id'] . " AND cc_invoice.date >= TIMESTAMP('$checkdate') AND cc_invoice.date <= CURRENT_TIMESTAMP
		GROUP BY MONTH(date)
		ORDER BY date DESC";
